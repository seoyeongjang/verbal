...
이 문서는 Verbal 앱을 일반 사용자에게 공개 노출하기 전까지 완료된 출시
준비 상태를 기록합니다.

## 완료된 항목

- 앱 이름과 출시 자료를 Verbal 기준으로 정리했습니다.
- Android release AAB를 준비하고 Google Play 내부 테스트 트랙에 업로드했습니다.
...
- 음성 메시지, STT transcript 처리, pending 음성 메시지 최종화, 음성 파일
  보존 정책 흐름.
- 캘린더 일정, 음성 일정 생성, 일정 알림, 아침 브리핑, 외부 캘린더 연동,
  공휴일 지원, 채팅방 일정 제안.
- 메시지 수정/삭제, 고정 메시지 동작, 메시지 박스 크기, 재생 버튼, 최신
  메시지 스크롤 등 메시징 UX 개선.
- 홈 메뉴와 설정 IA 개선: 내 프로필, 요청 메시지, 고객지원, 약관/정책,
  데이터 및 저장공간, 개인정보 및 보안, 언어/테마, 계정 관리.
- 오픈채팅 친구 초대 및 링크 공유 지원.
- Firebase Functions, Firestore rules, Storage rules, 출시 검증 스크립트,
  Play Console 지원 산출물.
- 플러그인 플랫폼 기획 및 초기 service/API 구조.

## 실행한 검증

- `npm run record:app-content-submitted`
  - 결과: App content 게이트를 완료로 기록했습니다.
...
- `npm run verify:launch-evidence`
  - 외부 증거가 아직 남아 있으므로 미완료 결과가 정상입니다.
...
Verbal은 내부 테스트 진행 단계까지 준비되어 있습니다. 일반 사용자 공개
노출은 아직 준비되지 않았으며, 위 4개 외부 게이트를 완료하고
`npm run verify:public-release-gate`가 통과할 때까지 계속 차단해야 합니다.
